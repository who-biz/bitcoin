...
*Setup instructions for a Gitian build of MIL Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MIL
...
- [Building MIL Core](#building-einsteinium-core)
...
Clone the git repositories for MIL and Gitian.
...
in the MIL repository under 'Fetch and create inputs' to install sources which require
...
To build MIL Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the MIL repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/einsteinium-project/einsteinium
...
./bin/gbuild --commit einsteinium=${COMMIT} --url einsteinium=${URL} ../MIL/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the MIL git repository with the desired tag must both be available locally, and then gbuild must be
...
[gitian.sigs.ltc](https://github.com/einsteinium-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
