...
void komodo_notarized_update(int32_t nHeight,int32_t notarized_height,uint256 notarized_hash,uint256 notarized_desttxid,uint256 MoM,int32_t MoMdepth)
{
    ...
    if ( fp != 0 )
    {
        if ( fwrite(np,1,sizeof(*np),fp) == sizeof(*np) )
            fflush(fp);
        else LogPrintf(">>> %s: error writing notarization to %d\n",__func__,(int32_t)ftell(fp));
    }
...
void komodo_connectblock(CBlockIndex *pindex,CBlock& block)
{
    ...
    LogPrintf(">>> %s: numnotaries(%d) \n",__func__,numnotaries);
    ...
    if ( pindex != 0 )
    {
        ...
        for (i=0; i<txn_count; i++)
        {
            ...
            LogPrintf(">>> %s: txhash(%s), nextblockheight(%d), numvins(%d), numvouts(%d)\n",__func__,txhash.GetHex(),(pindex->nHeight+1),numvins,numvouts);
            for (j=0; j<numvins; j++)
            {
                if ( i == 0 && j == 0 )
                    continue;
                ...
                }  else if ( block.vtx[i]->vin[j].prevout.hash != zero ) LogPrintf(">>> %s: %s cant get scriptPubKey for ht.%d txi.%d vin.%d, scriptlen(%d)\n",__func__,ASSETCHAINS_SYMBOL,height,i,j,scriptlen);
            }
            numvalid = bitweight(signedmask);
            if (testnet_enabled())
            {
                LogPrintf(">>> %s: numvalidfrommask(%d), TESTNET_MINRATIFY(%d)\n",__func__,numvalid,TESTNET_MINRATIFY);
